...
describe("scanConfigSchema", () => {
  ...
  it("rejects a config without budgets", () => {
    const config =
      require("../__fixtures__/scan-config-no-budgets.json") as object;
    const { error } = scanConfigSchema.validate(config);
  ...
  it("rejects a config with invalid budgets", () => {
    const config =
      require("../__fixtures__/scan-config-invalid-budgets.json") as object;
    const { error } = scanConfigSchema.validate(config);
  ...
  it("rejects a config with more invalid budgets", () => {
    const config =
      require("../__fixtures__/scan-config-more-invalid-budgets.json") as object;
    const { error } = scanConfigSchema.validate(config);
  ...
  it("rejects a config with invalid reporters", () => {
    const config =
      require("../__fixtures__/scan-config-invalid-reporters.json") as object;
    const { error } = scanConfigSchema.validate(config);
