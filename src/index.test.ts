...
describe('toobig', () => {
  ...
  describe('not passing a config file', () => {
    it('finds a config file', async () => expect(tooBig()).resolves.toBeUndefined());
  ...
  describe('passing an invalid config file', () => {
    ...
    it('throws', async () => {
      try {
        ...
        expect(err.message).toMatch(/JSON Error in .+\/src\/__fixtures__\/toobig.invalid.json/);
      }
    });
  });
  ...
  describe('passing a non-existent config file', () => {
    ...
    it('throws', async () => {
      try {
        ...
        expect(err.message).toMatch(/no such file or directory, open .+\/src\/__fixtures__\/toobig.notfound.json'/);
      }
    });
  });
});
